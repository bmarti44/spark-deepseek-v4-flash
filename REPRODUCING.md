...
.venv-harness/bin/python -m pip install -r requirements-harness.lock
.venv-harness/bin/python -m pip freeze | diff -u requirements-harness.lock -
```

[requirements-harness.lock](requirements-harness.lock) is the exact known-good
resolution captured from `.venv-harness`. [requirements-harness.txt](requirements-harness.txt)
records the two direct dependencies; use the lock, including transitive dependencies, for
reproduction. Regenerate the lock only in a separately reviewed dependency-update batch.
...
memory/CPU/PID limits, and a timeout. Follow NVIDIA's
[DGX OS Docker and NVIDIA Container Toolkit instructions](https://docs.nvidia.com/dgx/dgx-os-7-user-guide/installing_on_ubuntu.html#installing-docker-and-the-nvidia-container-toolkit),
which use the DGX repositories rather than the Docker snap. The evaluated host used
`Docker version 29.2.1, build a5c7197`. Install the supported packages, ensure the trusted
benchmark operator can invoke Docker, and verify its presence before an accuracy run:

```bash
sudo apt-get update
sudo apt-get install -y docker-ce nvidia-container-toolkit nv-docker-options
...
mkdir -p "$PWD/build/ds4" "$PWD/build/llamacpp"
sudo install -o "$(id -un)" -g "$(id -gn)" -m 0644 \
  /home/dsv4/ds4-project/build-manifest.json \
  "$PWD/build/ds4/build-manifest.json"
sudo install -o "$(id -un)" -g "$(id -gn)" -m 0644 \
  /home/dsv4/llamacpp-project/build-manifest.json \
  "$PWD/build/llamacpp/build-manifest.json"
```
...
The builders write live manifests to `/home/dsv4/ds4-project/build-manifest.json` and
`/home/dsv4/llamacpp-project/build-manifest.json`. The commands above preserve those
reproducer-generated bytes at stable paths under `$PWD/build/`; fetched weight manifests
remain beside the weights. Set `BUILD_MANIFEST` and `WEIGHTS_MANIFEST` to those generated
files immediately before each candidate's accuracy sequence, as shown below.

> **Fresh-run evidence boundary:** [configs/build-manifests/](configs/build-manifests/)
> contains **our** frozen evidence. For benchmark evidence it is used only by the offline
> audit (`scripts/36_audit_accuracy.py`) to verify **our** published verdict; the serve
> wrappers also use its binary hashes as a launch allowlist. Passing a committed manifest
> from that directory to a fresh benchmark command is wrong: the fresh run must bind the
> manifest generated by the reproducer's own build and its fetched weights.
...
```bash
sudo -u dsv4 -H env DSV4_VERIFY_WEIGHTS=full API_KEY_FILE=/home/dsv4/.config/deepseek-v4-flash/api-key ./scripts/21_serve_llamacpp.sh start
...
Both always check model byte sizes. The llama.cpp wrapper logs its default size-only mode;
`DSV4_VERIFY_WEIGHTS=full` adds all three shard hashes, matching ds4's `--full-verify`.
...
Define this local check once. It binds each speed sequence to the reproducer-generated
manifest even though script 30 has no `--config-evidence` option:
...
manifest_path, binary_path, binary_name = sys.argv[1:]
expected = json.load(open(manifest_path, encoding="utf-8"))["binaries"][binary_name]["sha256"]
digest = hashlib.sha256()
with open(binary_path, "rb") as stream:
    for chunk in iter(lambda: stream.read(1024 * 1024), b""):
        digest.update(chunk)
if digest.hexdigest() != expected:
    raise SystemExit(f"binary does not match reproducer manifest: {binary_path}")
PY
}
```
...
export BUILD_MANIFEST="$PWD/build/ds4/build-manifest.json"
verify_built_binary "$BUILD_MANIFEST" /home/dsv4/ds4-project/src/ds4/ds4-server ds4-server
...
export BUILD_MANIFEST="$PWD/build/llamacpp/build-manifest.json"
verify_built_binary "$BUILD_MANIFEST" /home/dsv4/llamacpp-project/src/llama.cpp/build/bin/llama-server llama-server
...
```bash
export BUILD_MANIFEST="$PWD/build/llamacpp/build-manifest.json"
export WEIGHTS_MANIFEST="$PWD/weights/unsloth-ud-q2_k_xl/manifest.json"
test -r "$BUILD_MANIFEST" && test -r "$WEIGHTS_MANIFEST"
...
prepare Tailscale **before** hardening, installing, or serving the product. Use Tailscale's
[official Ubuntu 24.04 package repository](https://pkgs.tailscale.com/stable/#ubuntu-noble);
APT selects its arm64 package on this aarch64 host:
...
sudo tailscale up
tailscale status
TAILSCALE_VERSION=$(tailscale version | sed -n '1p')
dpkg --compare-versions "$TAILSCALE_VERSION" ge 1.98.9
```

Complete the browser login printed by `tailscale up`; `tailscale status` must show this
authenticated node. Version 1.98.9 is the minimum for this deployment because it includes
the July 2026 Serve security fixes. Tailscale Serve also requires HTTPS certificates to be
enabled for the tailnet: enable MagicDNS and **HTTPS Certificates** in the tailnet DNS admin
page before continuing (see [Tailscale's HTTPS setup](https://tailscale.com/docs/how-to/set-up-https-certificates)).
The installer fails closed unless the authenticated CLI can report status and its
Serve/Funnel configuration.

Now create the separate no-login `dsv4auth` identity, remove the repository owner from the
