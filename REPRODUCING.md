...
This procedure assumes a fresh clone at the repository's fixed path,
`/home/bmarti44/spark-deepseek-v4-flash`. Several setup and systemd files contain that
absolute path and the account name `bmarti44`; a clone under another account or path is
not the configuration evaluated here. Commands below start in the repository root unless
stated otherwise.
...
As `bmarti44`, inspect the stock host before setup:
...
```bash
cd /home/bmarti44/spark-deepseek-v4-flash
sudo bash setup/01-bwrap-apparmor.sh
sudo bash setup/02-dgxos-update.sh
```
...
15-second warning. It does not update EC, UEFI, or other firmware. Re-enter the clone after
the reboot and confirm the locked host state:

```bash
cd /home/bmarti44/spark-deepseek-v4-flash
./scripts/00_preflight.sh --out /tmp/preflight-after-update.json
sudo bash setup/03-dsv4-user.sh
```

Step 03 creates the unprivileged `dsv4` system account, delegates only passwordless
`bmarti44 -> dsv4` execution through `/etc/sudoers.d/dsv4-delegate`, creates `/run/dsv4`
through tmpfiles, creates `/etc/deepseek-v4-flash`, disables Ollama, adds `bmarti44` to the
...
```bash
sudo setfacl -m u:dsv4:--x /home/bmarti44
sudo setfacl -R -m u:dsv4:rX /home/bmarti44/spark-deepseek-v4-flash
sudo find /home/bmarti44/spark-deepseek-v4-flash -type d -exec setfacl -d -m u:dsv4:rX {} +
```
...
As `bmarti44`, create the isolated harness environment:
...
GGUF, encoder, and evalsets as `bmarti44`:
...
read access with an ACL). Every llama.cpp gate command in section 7 then needs
`--api-key-file /home/dsv4/.config/deepseek-v4-flash/api-key` appended (the soak command
already shows it). The ds4 engine has no native auth, so its gate commands take no key;
in production BOTH engines sit behind the authenticating proxy from section 8.
...
Run gates as `bmarti44` against only the currently resident candidate. The commands below
...
```bash
.venv-harness/bin/python scripts/32_golden_tests.py --base-url http://127.0.0.1:8011 --out results/golden-llamacpp.json --stack-label llamacpp-udq2kxl --extra-body '{"chat_template_kwargs":{"enable_thinking":false}}'
.venv-harness/bin/python scripts/33_token_parity.py --backend llamacpp --base-url http://127.0.0.1:8011 --out results/parity-llamacpp.json
.venv-harness/bin/python scripts/30_bench_speed.py --base-url http://127.0.0.1:8011 --out results/speed-llamacpp.json --stack-label llamacpp-udq2kxl --reps 5 --warmup 1 --ignore-eos-supported --extra-body '{"chat_template_kwargs":{"enable_thinking":false}}'
```
...
```bash
.venv-harness/bin/python scripts/31_bench_accuracy.py --base-url http://127.0.0.1:8011 --out results/acc-gsm8k-dev-llamacpp.json --stack-label llamacpp-udq2kxl --extra-body '{"chat_template_kwargs":{"enable_thinking":false}}' --suite gsm8k --split dev --transcripts-dir results/transcripts/gsm8k-dev-llamacpp --config-hash llamacpp-32e789fd-udq2kxl-ctx32768-nothink-v1 --config-evidence configs/build-manifests/llamacpp.json weights/unsloth-ud-q2_k_xl/manifest.json
.venv-harness/bin/python scripts/31_bench_accuracy.py --base-url http://127.0.0.1:8011 --out results/acc-gsm8k-holdout-llamacpp.json --stack-label llamacpp-udq2kxl --extra-body '{"chat_template_kwargs":{"enable_thinking":false}}' --suite gsm8k --split holdout --transcripts-dir results/transcripts/gsm8k-holdout-llamacpp --config-hash llamacpp-32e789fd-udq2kxl-ctx32768-nothink-v1 --config-evidence configs/build-manifests/llamacpp.json weights/unsloth-ud-q2_k_xl/manifest.json
.venv-harness/bin/python scripts/31_bench_accuracy.py --base-url http://127.0.0.1:8011 --out results/acc-mmlu-dev-llamacpp.json --stack-label llamacpp-udq2kxl --extra-body '{"chat_template_kwargs":{"enable_thinking":false}}' --suite mmlu-pro --split dev --transcripts-dir results/transcripts/mmlu-dev-llamacpp --config-hash llamacpp-32e789fd-udq2kxl-ctx32768-nothink-v1 --config-evidence configs/build-manifests/llamacpp.json weights/unsloth-ud-q2_k_xl/manifest.json
.venv-harness/bin/python scripts/31_bench_accuracy.py --base-url http://127.0.0.1:8011 --out results/acc-mmlu-holdout-llamacpp.json --stack-label llamacpp-udq2kxl --extra-body '{"chat_template_kwargs":{"enable_thinking":false}}' --suite mmlu-pro --split holdout --transcripts-dir results/transcripts/mmlu-holdout-llamacpp --config-hash llamacpp-32e789fd-udq2kxl-ctx32768-nothink-v1 --config-evidence configs/build-manifests/llamacpp.json weights/unsloth-ud-q2_k_xl/manifest.json
.venv-harness/bin/python scripts/31_bench_accuracy.py --base-url http://127.0.0.1:8011 --out results/acc-humaneval-llamacpp.json --stack-label llamacpp-udq2kxl --extra-body '{"chat_template_kwargs":{"enable_thinking":false}}' --suite humaneval --split all --transcripts-dir results/transcripts/humaneval-llamacpp --config-hash llamacpp-32e789fd-udq2kxl-ctx32768-nothink-v1 --config-evidence configs/build-manifests/llamacpp.json weights/unsloth-ud-q2_k_xl/manifest.json
```
...
Finally generate the mechanical decision. It fails closed while either candidate lacks
required evidence and writes `results/decision.json` plus `results/DECISION.md` only when
validation reaches that point:
...
## 8. Install the winner in production

Do this only after `results/DECISION.md` names a winner. As root, create the separate
no-login `dsv4auth` identity, remove `bmarti44` from the `dsv4` group, restrict the key
directory, install Caddy, and install exactly the selected stack:

```bash
cd /home/bmarti44/spark-deepseek-v4-flash
sudo bash setup/04-production-hardening.sh
sudo apt-get update
sudo apt-get install -y caddy
```

Then run exactly one of these, matching the decision:
...
and rejects a Tailscale Serve configuration that mentions engine ports 8011 or 8012.
...
accuracy, transcript, soak, audit, exception, preflight, and ledger files present there.
Do not compare against numbers copied into another document. The decision is currently
pending because the full required evidence for both candidates is not yet present; the
decision tool is expected to fail closed until it is.
