...
But the methodology is not sound enough: the supposedly frozen gates were modified after failures, MMLU is materially over-scored, holdout secrecy is bypassable, A’s 5/5 long-context failure is ignored by the decision rule, and the credential/process isolation is not production-grade. The decision command currently fails closed because all six required final accuracy inputs are missing, which is the correct behavior and also means there is no winner yet.
